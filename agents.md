...
- **Prohibición de Supresión de Advertencias (filterwarnings):** Está estrictamente prohibido utilizar `filterwarnings` en `pyproject.toml` o bloques de supresión de advertencias (como `warnings.catch_warnings()`) en el código de importaciones. Las advertencias de obsolescencia (`DeprecationWarning`) deben ser visibles para identificar con claridad problemas de obsolescencia y forzar la migración hacia las alternativas modernas y mantenidas del stack.